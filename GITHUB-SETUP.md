...
Push/upload everything to the **root of the repo** — every file sits at the
top level, no subfolders:

```
index.html, about.html, programs.html, contact.html, donate.html, admin.html
app.js, style.css, admin.css, admin.js, site-content.js, github-config.js
home.json, about.json, contact.json, donate.json, team.json, programs.json, gallery.json
```

The `*.json` files are already pre-filled with the site's current content,
so nothing looks empty on day one.

**If you're uploading via GitHub's website on a phone:** use "Add file →
Upload files" and drag/select all these files in one go, directly at the
repo root. Don't put any of them inside a subfolder — the code expects them
all to sit next to each other at the top level.
