...
document.querySelector('[data-save="home"]').addEventListener("click", () => {
  saveDoc("home.json", homeMapping, "home-status", "Update home page content");
});
...
document.querySelector('[data-save="about"]').addEventListener("click", () => {
  saveDoc("about.json", aboutMapping, "about-status", "Update about page content");
});
...
document.querySelector('[data-save="contact"]').addEventListener("click", () => {
  saveDoc("contact.json", contactMapping, "contact-status", "Update contact info");
});
...
document.querySelector('[data-save="donate"]').addEventListener("click", () => {
  saveDoc("donate.json", donateMapping, "donate-status", "Update donation page content");
});
...
document.getElementById("team-add-btn").addEventListener("click", async () => {
  ...
  if (!name || !role) return;
  const list = (await ghGetJson("team.json")) || [];
  list.push({ name, role, image, order: list.length });
  await ghPutJson("team.json", list, `Add team member: ${name}`);
...
async function renderTeamList() {
  ...
  const list = ((await ghGetJson("team.json")) || []).map((d, i) => ({ ...d, _i: i }));
  ...
  list.forEach((d) => {
    ...
    card.querySelector(".save-item").addEventListener("click", async () => {
      const fresh = (await ghGetJson("team.json")) || [];
      fresh[d._i] = {
      ...
      await ghPutJson("team.json", fresh, `Update team member: ${fresh[d._i].name}`);
      renderTeamList();
    });
    card.querySelector(".delete-item").addEventListener("click", async () => {
      ...
      const fresh = (await ghGetJson("team.json")) || [];
      fresh.splice(d._i, 1);
      await ghPutJson("team.json", fresh, `Remove team member: ${d.name}`);
      renderTeamList();
    });
...
document.getElementById("program-add-btn").addEventListener("click", async () => {
  ...
  const list = (await ghGetJson("programs.json")) || [];
  list.push({ title, text, image, order: list.length });
  await ghPutJson("programs.json", list, `Add program: ${title}`);
...
async function renderProgramsList() {
  ...
  const list = ((await ghGetJson("programs.json")) || []).map((d, i) => ({ ...d, _i: i }));
  list.sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
  container.innerHTML = "";
  list.forEach((d) => {
    const card = document.createElement("div");
    ...
    `;
    card.querySelector(".save-item").addEventListener("click", async () => {
      const fresh = (await ghGetJson("programs.json")) || [];
      fresh[d._i] = {
      ...
      await ghPutJson("programs.json", fresh, `Update program: ${fresh[d._i].title}`);
      renderProgramsList();
    });
    card.querySelector(".delete-item").addEventListener("click", async () => {
      ...
      const fresh = (await ghGetJson("programs.json")) || [];
      fresh.splice(d._i, 1);
      await ghPutJson("programs.json", fresh, `Remove program: ${d.title}`);
      renderProgramsList();
    });
    container.appendChild(card);
  });
...
document.getElementById("activity-add-btn").addEventListener("click", async () => {
  ...
  const list = (await ghGetJson("gallery.json")) || [];
  list.push({ title, imageUrl, date: date || new Date().toISOString().slice(0, 10) });
  await ghPutJson("gallery.json", list, `Add gallery photo: ${title}`);
...
async function renderGalleryList() {
  ...
  const list = ((await ghGetJson("gallery.json")) || []).map((d, i) => ({ ...d, _i: i }));
  ...
  list.forEach((d) => {
    ...
    `;
    card.querySelector(".save-item").addEventListener("click", async () => {
      const fresh = (await ghGetJson("gallery.json")) || [];
      fresh[d._i] = {
      ...
      await ghPutJson("gallery.json", fresh, `Update gallery photo: ${fresh[d._i].title}`);
      renderGalleryList();
    });
    card.querySelector(".delete-item").addEventListener("click", async () => {
      ...
      const fresh = (await ghGetJson("gallery.json")) || [];
      fresh.splice(d._i, 1);
      await ghPutJson("gallery.json", fresh, `Remove gallery photo: ${d.title}`);
      renderGalleryList();
    });
    container.appendChild(card);
  });
...
async function loadEverything() {
  await loadDocInto("home.json", homeMapping);
  await loadDocInto("about.json", aboutMapping);
  await loadDocInto("contact.json", contactMapping);
  await loadDocInto("donate.json", donateMapping);
  renderTeamList();
  renderProgramsList();
  renderGalleryList();
}
